...
owns workflow orchestration and cross-engine bridges, while `qe-mcp` owns the
external expert corpus, pack metadata, and passive artifact-analysis surface.
Without `qe-mcp`, core QE skills still load, but MCP expert lookup is
unavailable.

The default user-facing MCP exposes compact expert search, explicit full-read
tools, passive help, and artifact analysis:
...
Legacy runner tools such as `qe_run_codex_agent`, `qe_run_claude_agent`, and
`qe_delegate_agent` are compatibility-only. They are hidden unless the MCP
server starts with `QE_MCP_EXPOSE_RUNNERS=1`; new SIVS execution should use the
framework bridge layer instead.
