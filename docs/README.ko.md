...
| Qtranslate | CJK, 라틴, 키릴, 아랍 문자 등의 프로그래밍적 언어 감지와 자동 language.md 관리를 지원하는 다국어 지원. |
...
| `UserPromptSubmit` | 사용자 메시지 전송 | CJK/bigram 매칭 기반 **인텐트 자동 분류** + Qtranslate용 **언어 감지** |
...
├── skills/            # 63개 core + 72개 coding expert 스킬 (Q-prefix)
├── core/              # 공유 원칙 및 설정
├── hooks/             # 라이프사이클 훅
├── install.js         # 설치 스크립트
└── package.json
