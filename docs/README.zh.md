...
> 没有规格的工作是猜测。没有验证的规格是希望。没有监理的验证是确认偏差。

QE Framework 围绕 **SVS 循环**（Spec → Verify → Supervise）构建：
...
1. **`/Qgenerate-spec`** -- 创建 TASK_REQUEST + VERIFY_CHECKLIST。Plan agent 在你查看之前自动审查规格。可执行性验证确保每个项目都是可操作的。
2. **`/Qrun-task`** -- 逐项执行检查清单并跟踪进度。显示任务类型横幅（CODE/DOCS/ANALYSIS），让你在批准之前清楚知道将要发生什么。
3. **Supervision** -- 领域专用监理代理（代码质量、安全、文档、分析）进行独立审查。如果未通过，系统自动生成 REMEDIATION_REQUEST 并重新运行循环 -- 最多 3 次，无需人工干预。
4. **最少打断** -- 你只在 5 个节点被询问：规格确认、执行提示、任务批准、完成报告、以及 3 次监理失败升级。其他一切都是自动的。

工程原则：SOLID、DRY、KISS、YAGNI、基于证据的决策、最小变更、多语言（自动语言检测）。
...
| Qautoresearch | 受Karpathy autoresearch启发的自主实验循环。支持多文件修改，集成Ecode-reviewer/debugger进行质量验证。反复修改代码、评估指标、keep/discard判定。 |
| Qmcp-setup | 自动检测、安装和配置项目所需的MCP服务器。扫描代码库推断需求并生成settings.json。 |
| Qstitch-cli | Google Stitch MCP 设置和 CLI 指南，用于 AI 驱动的 UI 设计。 |
| Qcc-setup | Claude Code shell 别名设置（cc、ccc、ccd），用于快速终端启动。 |
...
框架使用 9 个在特定事件触发的生命周期钩子：

| Hook | 触发时机 | 执行内容 |
|------|----------|----------|
| `SessionStart` | 会话开始时 | 注入框架规则，如果分析数据过时则触发 `Erefresh-executor` |
| `PreToolUse` | 每次工具调用前 | Intent Gate 路由显示、密钥扫描、上下文压力警告 |
| `PostToolUse` | 每次工具调用后 | 错误追踪/升级、工具调用计数、`Eprofile-collector` 触发 |
| `PreCompact` | 上下文压缩前 | 触发 `Ecompact-executor` 在上下文丢失前保存 |
| `Stop` | 会话结束时 | 会话日志记录、活跃工作的模式阻断 |
| `Notification` | 后台代理完成时 | 在后台代理完成后链式触发后续操作 |
| `UserPromptSubmit` | 用户发送消息时 | CJK/bigram 意图自动分类 + **语言检测** (保存到 `.qe/profile/language.md`) |
| `TaskCompleted` | 任务完成时 | 验证检查清单完成情况 |
| `TeammateIdle` | 代理团队成员空闲时 | 提示空闲队友认领待处理任务 |
...
| Esupervision-orchestrator | 监理编排器。路由到领域监理代理，汇总 PASS/PARTIAL/FAIL 评分，失败时自动生成 REMEDIATION_REQUEST。 |

### MEDIUM 层级 (sonnet)

| Agent | 描述 |
|-------|------|
| Etask-executor | 按顺序实现检查清单项目。支持基于 Wave 的独立项目并行执行。 |
...
| Ecode-quality-supervisor | 代码质量审计监理。审查代码质量、测试覆盖率、架构一致性。返回 PASS/PARTIAL/FAIL。 |
| Edocs-supervisor | 文档审计监理。审查完整性、准确性、结构一致性和链接有效性。返回 PASS/PARTIAL/FAIL。 |
| Eanalysis-supervisor | 分析审计监理。审查证据充分性、逻辑有效性、范围充分性和可操作性。返回 PASS/PARTIAL/FAIL。 |
| Esecurity-officer | 安全审计专家。扫描 git diff 变更中的漏洞，分类为 PASS/WARN/FAIL。 |
| Edocs-collector | 从任务上下文中提取领域知识并记录到 .qe/docs/。 |
...
qe-framework/
├── .claude-plugin/    # 插件配置
├── agents/            # 22 个代理 (E-prefix)
├── skills/            # 65 个核心 + 72 个编码专家技能 (Q-prefix)
├── core/              # 共享原则与配置
├── hooks/             # 生命周期钩子
├── install.js         # 安装脚本
└── package.json
