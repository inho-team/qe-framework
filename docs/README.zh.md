...
| Qtranslate | 多语言翻译，支持语法纠正。编程式语言检测（CJK、拉丁、西里尔、阿拉伯文字）并自动管理language.md。 |
...
| `UserPromptSubmit` | 用户发送消息时 | CJK/bigram 意图自动分类 + Qtranslate 语言检测 |
...
├── skills/            # 63 个核心 + 72 个编码专家技能 (Q-prefix)
├── core/              # 共享原则与配置
├── hooks/             # 生命周期钩子
├── install.js         # 安装脚本
└── package.json
