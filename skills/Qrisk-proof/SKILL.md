---
name: Qrisk-proof
description: "Use for code tasks after Verify and before Supervise to prove risks were actively challenged. Audits TASK_REQUEST Risk Register, changed files, tests, and evidence with Erisk-proof-auditor; returns PASS/WARN/FAIL. Auto-invoked by Qcode-run-task for type: code."
user_invocable: true
invocation_trigger: "When the user invokes Qrisk-proof, when Qcode-run-task reaches the risk proof gate for a code task, or when a code change needs evidence-backed risk validation before Supervise."
recommendedModel: sonnet
tier: core
---

# Qrisk-proof — Evidence-Backed Code Risk Gate

## Role

`Qrisk-proof` turns a code task's Risk Register into a proof-oriented gate. It
does not ask whether the report sounds careful. It asks whether each meaningful
risk was challenged with a probe, evidence, and a clear status before Supervise.
