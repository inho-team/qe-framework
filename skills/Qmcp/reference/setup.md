---
source-skill: Qmcp-setup
subcommand: Qmcp setup
---
...
This subcommand is a **setup and configuration guide only**. It does NOT execute
MCP operations.

| Request | Correct action |
|---------|---------------|
| "Set up MCP server", "connect Google Drive" | **This subcommand** - guide setup steps |
| "Get files from Google Drive", "Send Slack messages" | **NOT this subcommand** - use connected MCP tools directly |

### Pre-check: MCP Server Status

Before guiding setup, check if the requested MCP server is already connected
through the active client adapter.
...
- If no native MCP surface is available, document the server config for manual
  installation and do not claim Claude CLI commands will configure Codex.

**If connected**: "Already connected. Use MCP tools directly." - exit skill.
**If NOT connected**: proceed with setup guide.

## Role
Guides users through discovering, installing, and configuring MCP servers to
connect the active AI client with external services.
...
If the service is not listed, search npm/PyPI/GitHub for
`mcp-server-{service}` or `{service}-mcp`.
...
See references/service-setup-guides.md for detailed setup instructions for
Google Drive, GitHub, Slack, PostgreSQL, and Filesystem.

## QE Framework MCP Catalog

These MCPs have dedicated setup skills in the framework. Use the dedicated skill
for detailed guidance, or set up quickly from here.
...
> For services not in this catalog (Google Drive, Slack, GitHub, etc.), see the
> general setup guide below.
...
When using Agent Teams (`--agents` flag), each teammate can have its own MCP
...
> Use this section when building MCP servers from scratch. This is a
> **development guide only** - it does NOT auto-generate server code.
...
Pre-configured MCP server sets are available in `presets/`. Offer these to the
user before building from scratch:
...
Usage: Copy the selected preset's `mcpServers` block into the project's
`.claude.json` or MCP config file. Replace `${PROJECT_ROOT}` and other
