...
- ★는 **결론 바로 아래 `근거 수준:` 한 줄에만** 둔다 — 답변당 1곳(Tier 2 #7과 동일). 항목마다 ★를 흩지 않는다.
...
**라벨은 세션 언어로.** 사용자에게 보이는 선택지 라벨은 세션 언어를 따른다 — 한국어 세션은 동작명을 우리말로(`그냥 실행` / `병렬 실행(웨이브)` / `스펙 생성` / `스펙 수정 후 재검사`), 영어 세션은 영어 라벨 유지(`Generate & Execute` 등). 괄호 안 명령(`/Qrun-task`)과 내부 라우팅 키는 그대로 둔다. `Generate Only` 같은 영어 폼을 한국어 세션에 노출하지 않는다.
...
위치는 고정 — **결론 바로 아래 `근거 수준:` 한 줄**(고정 뼈대 참조). 답변당 1곳.
...
8. 등장한 섹션이 **고정 뼈대 순서**를 따르나? 표는 파이프 표 한 종류, ★는 `근거 수준:` 1곳인가?
