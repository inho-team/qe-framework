...
- **Qexecute -utopia (autonomous mode) check**: Utopia changes confirmation behavior only inside an explicitly entered Full SIVS Plan; it never promotes an ordinary request into Full SIVS. Before prompting through the interaction adapter in that Plan, check `.qe/state/utopia-state.json`. If `enabled: true`, skip non-destructive confirmations and auto-select the first recommended option. It does NOT skip destructive git operations or file deletions outside `.qe/`.
- **Pre-execution Gate**: Inside Qexecute -utopia (with or without `-verify`), check whether the already planned task has concrete anchor signals such as file paths, function names, or issue numbers. A vague task returns to the Plan-owned specification step; it does not expose an internal skill command or create a Full SIVS entry from ordinary prose. See the "Pre-execution Gate" section in Qgenerate-spec SKILL.md for details.
